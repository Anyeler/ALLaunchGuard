...
仓库 SHALL 提供通用示例工程（git 追踪）：以本地 SPM 包方式引用本库；正常运行时首页 SHALL 展示"本次启动已执行的启动任务"清单；SHALL 提供"模拟启动闪退"操作（DEBUG 限定，进程立即崩溃）；SHALL 提供"直接进入安全模式"调试操作；SHALL 注册内置清缓存动作与至少一个自定义修复动作；SHALL 注册 safeModeLaunchTasks 最小启动任务演示（纯 Foundation 模拟日志模块初始化，两个任务按注册顺序排列，展示注册顺序与任务依赖语义；不依赖任何启动编排器）。模拟闪退累计达到闪退阈值时，该次启动 SHALL 直接进入安全模式（正常任务清单不展示、菜单页窗口接管；默认阈值 3：连续两次触发后第三次启动接管），且最小任务 SHALL 在安全模式 UI 安装前同步执行完毕。示例 SHALL 另提供"模拟连续启动闪退"开关（DEBUG 限定）：开启后自动崩溃剩余次数置 3，其后每次正常启动若剩余次数 > 0 则在 5 秒存活窗口内自动崩溃并递减；达到闪退阈值的那次启动 SHALL 直接进入安全模式并自动清零剩余次数（进入安全模式即结束演示、开关自动关闭）——默认阈值 3 下形成"开启开关 → 前两次启动自动崩溃 → 第三次启动安全模式接管并自动清零"的一键闭环演示（预支递增计数语义，适配任意阈值；修复并一键重启后完全恢复）。安全模式启动时最小任务执行 SHALL 留下一次性可见证据：修复重启回正常路径后，首页 SHALL 展示一次性横幅提示"上次安全模式启动已执行最小任务"，展示后清除（不常驻）。
...
#### Scenario: 安全模式最小启动任务演示
- **WHEN** 示例经任一路径进入安全模式（阈值触发 / 粘滞 / DEBUG 直入）
- **THEN** 两个最小任务按注册顺序在安全模式 UI 安装前同步执行（控制台先输出日志模块初始化、再输出安全模式启动记录），正常启动路径不执行该任务链；经修复重启回正常路径后，首页展示一次性横幅提示上次安全模式已执行最小任务，展示后不再重复出现
